...
| **`ci_full.sh`** | **Full** CI graph: protos, migrated binaries, **all** **`oci_image`** targets in the smoke set, **unit** tests (including **`//src/cart:cart_dotnet_test`**), **`bazel run //:lint`**. Parity with the pre-M4 inline **`bazel_smoke`** job. |
| **`ci_fast.sh`** | **Faster** loop: protos, smoke, libraries/publish trees, **no** heavy OCI image builds (skips **`frontend_image`**, **`*_oci_image`**, etc.). Use before push when you only changed logic/tests. |
...
```bash
chmod +x tools/bazel/ci/*.sh src/cart/run_cart_dotnet_test.sh   # once, if needed
