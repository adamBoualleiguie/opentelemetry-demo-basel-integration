...
**BZ-130 (test tags):** **`docs/bazel/test-tags.md`**; **`bazel test //... --config=unit`** runs only tests tagged **`unit`** (**`//src/checkout/money:money_test`**, **`//src/shipping:shipping_test`**, **`//src/flagd-ui:flagd_ui_mix_test`**, **`//src/quote:quote_composer_smoke_test`**, …). **`cart`** **`dotnet test`** is not wired as **`bazel test`** yet (**BZ-081**).
