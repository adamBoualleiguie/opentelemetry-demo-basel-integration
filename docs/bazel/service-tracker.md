...
**CI:** `.github/workflows/checks.yml` job **`bazel_smoke`** (`continue-on-error: true`) builds **`//src/checkout/...`**, **`//src/product-catalog/...`**, **`//src/payment/...`** (includes **`payment_image`**), **`//src/frontend:frontend_image`**, **`//pb:demo_py_grpc`**, **`//pb:demo_java_grpc`**, **`//src/ad:ad`**, **`//src/ad:ad_oci_image`**, **`//src/fraud-detection:fraud_detection`**, **`//src/fraud-detection:fraud_detection_oci_image`**, **`//src/accounting:accounting_publish`** (**.NET 10** via **`actions/setup-dotnet`**), the four Python **`py_binary`** targets and their **`*_image`** targets, runs Go tests for the two Go services, and runs **`bazel test //src/frontend:lint`** (**BZ-051**).
